...
function updateCartPanel(){
  ...
  cart.forEach((id,index)=>{
    ...
    item.innerHTML=`<span>${p.name}</span><div class="flex items-center gap-2"><span>$${p.price}</span><button class="text-red-500 hover:text-red-400" onclick="removeCartItem(${index})"><i data-lucide="trash-2" class="w-4 h-4"></i></button></div>`;
...
window.closeModal=function(){ modal.classList.remove("active"); modal.style.opacity="0"; modal.style.visibility="hidden"; }
...
document.getElementById("add-product-btn").addEventListener("click",()=>{
  ...
  reader.onload=e=>{
    products.push({id:products.length+1,name,price,description:desc,image:e.target.result,tags:["New"]});
    renderProducts();
  }
